...
class PlayerDetail(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        player_to_get = [player for player in players_dict if player['full_name'] == kwargs['search_string']]
                
        if player_to_get != []:
            player_obj_search = PlayerSearches.objects.filter(player=player_to_get[0]['full_name'])
            ...
            if player_obj_search:
                ...
                PlayerSearches.objects.create(player=player_to_get[0]['full_name'], search_count=1)
            
            if player_to_get[0]['is_active'] == False:
                player_stats = playercareerstats.PlayerCareerStats(player_id=player_to_get[0]['id'], per_mode36='Totals')
                
                player_stats_career = json.loads(player_stats.get_json())['resultSets'][1]
                            
                players_dict_with_stats[player_to_get[0]['full_name']] = player_stats_career['rowSet'][0]
                ...
                for i in range(3):
                    players_dict_with_stats[player_to_get[0]['full_name']].pop(i)
                
                career_total_points = 0
                career_total_assits = 0
                career_total_rebounds = 0
                
            player_first_name = player_to_get[0]['first_name']
            player_last_name = player_to_get[0]['last_name']
                        
            if player_to_get[0]['is_active'] == True:
            ...
            context = {
                'player': player_to_get[0],
            }
            
            if player_to_get[0]['is_active'] == False:
                context['stats'] = players_dict_with_stats
                context['headers'] = player_stats_career['headers']
                            
            if player_to_get[0]['is_active'] == True:
                context['url'] =  img_url
                context['team'] = team_name
                context['player_season_stats'] = dict_of_player_stats
        
            return render(request, self.template_name, context=context)
        else:
            messages.add_message(request, messages.ERROR, "Sorry, that player is not recognized. Please try again!")
